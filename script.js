...
// ============================================================
// SCROLL-REVEAL  (IntersectionObserver on .reveal* classes)
// ============================================================
(function () {
...
// ============================================================
// HERO CANVAS — floating color particles
// ============================================================
(function () {
  ...
  const PALETTE_HEX = [
    '#e8b4b8','#f2c5c0','#e8856a','#e8a84a','#d4a843',
    '#8faa8a','#a8d8ea','#5a8faa','#6a6a9a','#b4a8d8',
    '#f9c9a0','#d4c4a8'
  ];
  ...
  function draw() {
    ...
    particles.forEach((p, i) => {
      ...
      if (p.life >= p.maxLife || p.y < -10) {
        ...
        particles[i].y = H + 5; // rise from bottom
      }
    });
...
// ============================================================
// HERO HEADLINE — word-by-word float in
// ============================================================
(function () {
  ...
  h1.innerHTML = words.map((w, i) =>
    `<span class="hero-word" style="animation-delay:${0.1 + i * 0.1}s">${w}</span>`
  ).join(' ');
})();
...
// ============================================================
(function () {
  const MAGNETIC = 'button.btn-primary, button.btn-secondary, .submit-btn, .nav-login-btn';
...
// ============================================================
// CLICK RIPPLE — global ripple on any .btn-primary / .btn-secondary
// ============================================================
(function () {
  document.addEventListener('click', e => {
    ...
    ripple.style.cssText = `
      ...
      pointer-events:none;
    `;
    // btn needs relative positioning (already on most)
...
// ============================================================
// TYPEWRITER — random card text
// ============================================================
(function () {
  const textEl = document.getElementById('random-text');
  if (!textEl) return;

  let tw_raf, tw_cancel = false;

  function typewrite(text, el) {
    tw_cancel = true; // cancel previous
    cancelAnimationFrame(tw_raf);
    setTimeout(() => {
      tw_cancel = false;
      let i = 0;
      el.innerHTML = '';
      ...
      function step() {
        ...
        if (i <= text.length) {
          ...
          el.appendChild(cursor);
          i++;
          tw_raf = requestAnimationFrame(() => setTimeout(step, 18));
        }
      }
      step();
    }, 50);
  ...
  // Observe random-text for content changes and typewrite it
  const observer = new MutationObserver(() => {
    // Only fire when text was set directly (not by us)
    const text = textEl.textContent.replace(/\|$/, '').trim();
    if (text && !textEl.querySelector('.typewriter-cursor')) {
      typewrite(text, textEl);
    }
  });
  observer.observe(textEl, { childList: true, characterData: true, subtree: false });
})();

// ============================================================
// PALETTE DONUT — spin in on render
// ============================================================
(function () {
...
// ============================================================
// MESSAGE CARD — card-hover glow tint from accent color
// ============================================================
(function () {
  document.addEventListener('mouseover', e => {
    const card = e.target.closest('.message-card');
    if (!card) return;
    const dot = card.querySelector('.message-color-dot');
    if (dot) {
      const color = dot.style.background || dot.style.backgroundColor || '';
      if (color) {
        card.style.setProperty('--card-hover-glow',
          color.replace(')', ', 0.08)').replace('rgb', 'rgba')
            .replace('#', 'rgba(') || 'transparent'
        );
      }
    }
  });
...
// ============================================================
// COLOR FILTER — re-trigger bounce animation on select
// ============================================================
(function () {
  document.addEventListener('click', e => {
    const f = e.target.closest('.color-filter');
    if (!f) return;
    f.classList.remove('selected');
    void f.offsetWidth;
    // selected class is toggled by the existing code — just force reflow
  });
})();

// ============================================================
// ABOUT PAGE — re-run stagger when activated
// ============================================================
(function () {
  ...
  window.showPage = function(name) {
    orig(name);
    if (name === 'about') {
      // reset + re-trigger the CSS transition stagger
      const ps = document.querySelectorAll('#page-about p');
      ps.forEach(p => { p.style.opacity = '0'; p.style.transform = 'translateX(-16px)'; });
      requestAnimationFrame(() => {
        ps.forEach(p => { p.style.opacity = ''; p.style.transform = ''; });
      });
    }
  };
})();
